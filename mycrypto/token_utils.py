...
def transfer_main_token(from_wallet, to_wallet_address, amount):
    web3_client = get_web3_client()
    txn = {
        'to': to_wallet_address,
        'chainId': Web3.toHex(56),
        'value': web3_client.toWei(amount, 'ether'),
        'nonce': web3_client.eth.get_transaction_count(from_wallet.address),
...
def transfer_erc20_token(token_contract, from_wallet, to_wallet_address, amount):
    round_amount = round(amount * 10 ** 18)
    web3_client = get_web3_client()
    contract = get_token_contract(token_contract)
    txn = contract.functions.transfer(to_wallet_address, round_amount).buildTransaction({
...
def get_main_token_balance(address):
    return get_web3_client().eth.get_balance(address)

def get_erc20_token_balance(token_contract, address):
    return get_token_contract(token_contract).functions.balanceOf(address).call()
